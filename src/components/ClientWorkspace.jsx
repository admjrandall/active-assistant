...
const ClientWorkspace = ({ client, onClose, showToast, requestConfirm }) => {
  ...
  return (
    <>
      ...
      <div className={`fixed inset-y-0 right-0 w-full md:w-[600px] max-w-full acrylic-dark transition-transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] z-[10000001] ${isOpen ? 'translate-x-0' : 'translate-x-full'}`} role="dialog" aria-modal="true" aria-labelledby="client-workspace-title" aria-busy={isSaving}>
        <div className="flex flex-col h-full bg-white/95 md:rounded-l-2xl overflow-hidden">
          ...
          <div className="px-4 md:px-6 py-4 md:py-6 border-b border-slate-200/50 flex justify-between items-center gap-4 flex-shrink-0">
            <input id="client-name" name="name" value={formData.name} onChange={handleChange} placeholder="Enter Client Name" aria-label="Client name" maxLength={MAX_TITLE_LENGTH} className="text-2xl font-bold bg-transparent border-b border-transparent focus:border-indigo-500 focus:outline-none w-full min-w-0" />
            <button type="button" onClick={handleClose} aria-label="Close client workspace" disabled={isSaving} className="p-2 rounded-full hover:bg-slate-200 text-slate-500 disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0"><Icons.Close /></button>
          </div>
